...
from settings import from_mail, to_mail,ppt_url, sleep_time, tnp_pwd, entry_no, login_url
hash_file_name = 'hash_ppt'
...
while True:
    ...
    try:
        mailer = MailUtils()        
        mail_params = {'subject': 'You might have new PPTs to register !','from':from_mail,'to':to_mail}
        
        session = requests.Session()
        payload = {'username': entry_no, 'password': tnp_pwd, 'login': 'Submit'}
        response = session.post(login_url, data=payload, verify=False)

        page = session.get(ppt_url, verify=False)
        ...
        if mailer :
            mail_params['subject'] = 'TnP PPT sync script crashed !'
            mail_params['body'] = 'Unfortunately your TnP PPT sync script crashed !\nHere is the exception traceback\n{}'.format(log)
            mailer.send_mail(from_mail,to_mail,mail_params)
        if session:
            session.close()

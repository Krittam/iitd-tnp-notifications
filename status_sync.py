...
from settings import from_mail, to_mail,status_url, sleep_time, tnp_pwd, entry_no, login_url
hash_file_name = 'hash_status'
...
while True:
    ...
    try:
        mailer = MailUtils()        
        mail_params = {'subject': 'TnP Application status changed !','from':from_mail,'to':to_mail}
        
        session = requests.Session()
        payload = {'username': entry_no, 'password': tnp_pwd, 'login': 'Submit'}
        response = session.post(login_url, data=payload, verify=False)

        page = session.get(status_url, verify=False)
        ...
        if mailer :
            mail_params['subject'] = 'TnP status sync script crashed !'
            mail_params['body'] = 'Unfortunately your TnP status sync script crashed !\nHere is the exception traceback\n{}'.format(log)
            mailer.send_mail(from_mail,to_mail,mail_params)
        if session:
            session.close()
